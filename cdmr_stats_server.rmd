...
data <- read_rds("deck_data.rds")
all_decks_join <- read_rds("all_decks_join.rds")

# Set Factors
cols <- c("Game_ID","Game_Type", "Player", "Commander", "Win_Type", "MV_Card", "Deck_ID")
...
most_played_card <- all_decks_join %>% 
  filter(type_1 != "Land" | type_2 == "Land", category != "Ramp") %>% 
  group_by(card_name) %>%
  count() %>% 
  ungroup() %>% 
  arrange(desc(n)) %>%
...
land_count <- all_decks_join %>%
  filter(type_1 == "Land" | type_2 == "Land") %>% 
  group_by(deck_name, Deck_ID, Player) %>% 
  summarise(Sum_land_count = sum(Quantity, na.rm = T))


all_decks_join %>%
  filter(type_1 != "Land" | type_2 == "Land" ) %>% 
  #distinct(Deck_ID, .keep_all = TRUE) %>% 
  summarise(`Average land count` = mean(land_count$Sum_land_count),
            `Average cmc` = mean(cmc, na.rm = T),
            `Average salt score` = mean(salt, na.rm = T),
            `Average card price` = mean(price, na.rm = T),
            `Most expensive card` = max(price)) %>%
...
player.cols.df <- deck_cmc_average %>% 
  distinct(Player, .keep_all = T) %>% 
  select(Player, color) 

player.cols = as.character(player.cols.df$color)
names(player.cols) = player.cols.df$Player
...
# Game data
```{r, echo=FALSE}
DT::renderDataTable(data, filter = 'top')
```

# Deck data
```{r, echo=FALSE}
my_table <-  DT::renderDataTable(all_decks_join, filter = 'top', server = T)
my_table
...
### Win Turn by Win Type 

```{r Colorwin, echo=FALSE}
ggplotly(data[!is.na(data$Win_Type),] %>%
           filter(Win == 1) %>% 
           ggplot(aes(x = as.factor(Win_Type), y = Win_Turn, fill = Win_Type)) +
          geom_boxplot() +
           scale_fill_manual(values = player_color_mapping$color)+
           labs(x = "", y = "Win Turn") +
           theme_bw() + 
...
selectInput(
  "Player", label = "Player:",
  choices = c(unique(data$Player)), selected = "Ale")
...
renderTable({
  all_decks_join %>% 
  filter(type_1 != "Land", category != "Ramp", Player == input$Player) %>%  # input$Player
  group_by(card_name) %>%
  count() %>% 
  ungroup() %>% 
  arrange(desc(n)) %>%
...
```{r player stats3, echo=FALSE}
renderTable({
  land_count %>%
    filter(Player == input$Player) %>%  # input$Player
    group_by(Player) %>% 
    summarise(`Average land count` = mean(Sum_land_count, na.rm = T),
              `Median land count` = median(Sum_land_count, na.rm = T),
              `Max land count` = max(Sum_land_count, na.rm = T),
              `Min land count` = min(Sum_land_count, na.rm = T),
              ) %>% 
    select(-Player) %>% 
    as.data.table() })
...
### Average Mana Value Distribution
