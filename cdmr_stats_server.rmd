...
quantity_list <- lapply(json_data_list, function(i) {
  ...
  rename(deck_name = d,
         ...
         category = c,
         maybe = m) %>% 
    filter(str_detect(maybe, "Maybeboard", negate = TRUE)) # Filters out Maybeboard cards
  })
...
ggplotly(deck_cmc_average %>%
  ...
  geom_bar(position="dodge", stat="identity") + 
    scale_fill_manual(values = c(Aless = player_color_mapping$Color[1],
                                 Domi  = player_color_mapping$Color[2],
                                 Elie = player_color_mapping$Color[3],
                                 Guy = player_color_mapping$Color[4],
                                 Fubu = player_color_mapping$Color[5],
                                 Lueku = player_color_mapping$Color[6],
                                 Lucien = player_color_mapping$Color[7],
                                 Tobi = player_color_mapping$Color[8],
                                 Domitwo = player_color_mapping$Color[9])) +
    facet_wrap(~Player) + 
...
Hey there! 😄 Before we continue, I need a bit of your magic to complete the spell. Could you kindly whisper your enchanting name into the digital cauldron below.
...
renderPlot({
  ...
  geom_bar(position="dodge", stat="identity") + 
    ...
    # Handle the case where an error (player not in the list) occurs
    plot(NULL)
  })
})
```
